...
ARCHIVE_PATH = Path("data/kd_history.csv")   # one row per site per source_date
...
ARCHIVE_FIELDS = [
    "source_date", "site", "kd", "ocean_cells",
    "l3_observed_fraction", "l4_interpolated_fraction",
    "confidence", "fetched_utc", "age_days_at_fetch",
]


def append_archive(out):
    """Append one row per site to a permanent CSV, never rewriting old rows.

    Backfills are welcome; re-runs of the same day are not. Existing
    source_date + site pairs are skipped, so a manual re-run or a retried
    scheduled job cannot duplicate a day.
    """
    existing = set()
    if ARCHIVE_PATH.exists():
        with ARCHIVE_PATH.open(newline="") as fh:
            for row in csv.DictReader(fh):
                existing.add((row["source_date"], row["site"]))

    new_rows = [
        {
            "source_date": out["source_date"],
            "site": key,
            "kd": s["kd"],
            "ocean_cells": s.get("ocean_cells"),
            "l3_observed_fraction": s.get("l3_observed_fraction"),
            "l4_interpolated_fraction": s.get("l4_interpolated_fraction"),
            "confidence": s["confidence"],
            "fetched_utc": out["generated_utc"],
            "age_days_at_fetch": out["age_days"],
        }
        for key, s in out["sites"].items()
        if (out["source_date"], key) not in existing
    ]

    if not new_rows:
        print(f"{ARCHIVE_PATH}: {out['source_date']} already recorded, nothing appended")
        return

    ARCHIVE_PATH.parent.mkdir(parents=True, exist_ok=True)
    write_header = not ARCHIVE_PATH.exists()
    with ARCHIVE_PATH.open("a", newline="") as fh:
        w = csv.DictWriter(fh, fieldnames=ARCHIVE_FIELDS)
        if write_header:
            w.writeheader()
        w.writerows(new_rows)
    print(f"appended {len(new_rows)} rows to {ARCHIVE_PATH}")
